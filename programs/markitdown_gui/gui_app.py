...
class MarkItDownGUI:
    ...
    def convert_task(self, item_id):
        ...
        try:
            ...
            self.root.after(0, lambda n=name: self.res_list.insert(tk.END, n))
            self.root.after(
                0, lambda i=item_id: self.queue_tree.set(i, "status", "已完成")
            )
            ...
            self.root.after(
                0,
                lambda i=item_id, err=e: self.queue_tree.set(
                    i, "status", f"失败: {str(err)[:20]}"
                ),
            )
