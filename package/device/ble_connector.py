...
class BLEConnector:
    ...
    def _init_library(cls):
        try:
            ...
            cls._lib.ble_get_scan_results.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(BLEDeviceInfo),
                ctypes.c_int,
            ]
    ...
    def scan(self, duration_ms: int = 5000) -> Dict:
        if not self._fw:
            return {"results": [], "error": "Library not loaded"}
    ...
    def write(
        self, address: str, service: str, char: str, hex_data: str, fast: bool = False
    ) -> Dict:
        if not self._fw:
            return {"success": False}
