...
class LogManager:
    ...
    def _configure(cls, **kwargs):
        ...
        if cls._enable_file_logging:
            ...
            cls._add_file_handler(
                root_logger, "all.log", logging.NOTSET, default_formatter
            )
            ...
            # 5. ERROR 专用：所有错误和异常
            cls._add_file_handler(
                root_logger, "error.log", logging.ERROR, cls._get_formatter("error")
            )
            ...
            cls._add_file_handler(
                root_logger, "application.json", logging.INFO, json_formatter
            )
    ...
    def _add_file_handler(
        ...
        if cls._log_rotation == "time":
            handler = TimedRotatingFileHandler(
                filename=file_path, **cls._time_rotation_params
            )
        else:  # 默认为按大小轮转
    ...
    def configure(cls, **kwargs):
        ...
        if not cls._configured:
            ...
            raise RuntimeError(
                "日志系统已经初始化，无法重新配置。请在第一次获取 logger 之前调用 configure。"
            )
            ...
            for handler in logger.handlers:
                ...
                if old_formatter:
                    ...
                    handler.setFormatter(
                        logging.Formatter(new_format, datefmt=old_formatter.datefmt)
                    )
...
LogManager._custom_filters = {
    ...
    "sensitive_data_filter": lambda record: (
        not any(
