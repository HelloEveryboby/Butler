...
class ScheduleManager:
    ...
    def schedule_event(self, datetime_obj, event, repeat):
        if repeat == "每天":
            schedule.every().day.at(datetime_obj.strftime("%H:%M")).do(
                self.event_reminder, event
            )
        elif repeat == "每周":
            schedule.every().week.at(datetime_obj.strftime("%H:%M")).do(
                self.event_reminder, event
            )
        elif repeat == "每月":
            schedule.every().month.at(datetime_obj.strftime("%H:%M")).do(
                self.event_reminder, event
            )
