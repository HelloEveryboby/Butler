...
def run(*args, **kwargs):
    ...
    prog_info = extension_manager.code_execution_manager.get_program(
        "hybrid_system_executor"
    )
    ...
    try:
        ...
        with HybridLinkClient(executable, cwd=cwd) as client:
            ...
            if "error" not in stats:
                print(
                    f"   [调度状态] 活跃协程: {stats['goroutines']} | 待处理队列: {stats['pq_len']}"
                )
                print(
                    f"   [资源信息] 内存分配: {stats['alloc_mb']} MB | 系统占用: {stats['sys_mb']} MB"
                )
            ...
            scan_results = client.call(
                "log_scan", {"dir": log_dir, "regex": regex_pattern}, timeout=15
            )
            ...
            if isinstance(scan_results, dict) and "error" not in scan_results:
                ...
                print(
                    f"   ✅ 扫描完成! 在 {len(scan_results)} 个文件中发现 {match_count} 处匹配。"
                )
            ...
            print(
                f"   ✅ 高优先级任务即时返回 (当前队列长度: {quick_stats.get('pq_len')})"
            )
            ...
            if isinstance(nodes, list) and nodes:
                ...
                for node_str in nodes:
                    ...
                    dispatch_res = client.call(
                        "remote_dispatch", {"ip": ip, "cmd": "STATUS_CHECK"}, timeout=5
                    )
                    print(
                        f"      📩 远程响应: {dispatch_res.get('node_response', '请求超时')}"
                    )
            else:
