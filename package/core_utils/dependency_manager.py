...
def inject_absolute_paths():
    """
    强制进行基于项目根目录的绝对路径动态注入。
    防止便携式路径在子脚本或子进程中因为运行工作目录改变而报错。
    """
    try:
        # 获取最准确的项目根目录
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        lib_path = os.path.join(base_dir, "lib_external")
        ...
        # 针对第三方库，注入 site-packages
        import site
        site.addsitedir(lib_path)

        logger.info(f"绝对路径注入完成: lib_external -> {lib_path}")
    except Exception as e:
        logger.error(f"路径注入异常: {e}")
