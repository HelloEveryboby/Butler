...
class HealthMonitor:
    ...
    def __init__(self):
        ...
        self.bhl_mapping = {
            "hybrid_compute": {
                "lang": "C++",
                "source": "compute.cpp",
                "binary": "hybrid_compute_exec",
            },
            "hybrid_crypto": {
                "lang": "Rust",
                "source": "src/crypto_service.rs",
                "binary": "target/release/hybrid_crypto_exec",
            },
            "hybrid_net": {
                "lang": "Go",
                "source": "net_service.go",
                "binary": "hybrid_net_exec",
            },
            "hybrid_sysutil": {"lang": "C", "source": "sysutil.c", "binary": "sysutil"},
            "hybrid_terminal": {
                "lang": "Go",
                "source": "main.go",
                "binary": "terminal_service",
            },
            "hybrid_math": {
                "lang": "C++",
                "source": "src/math_service.cpp",
                "binary": "hybrid_math_exec",
            },
            "hybrid_vision": {
                "lang": "C++",
                "source": "src/vision_service.cpp",
                "binary": "hybrid_vision_exec",
            },
            "hybrid_doc_processor": {
                "lang": "C++",
                "source": "src/main.cpp",
                "binary": "processor",
            },
    ...
    def generate_full_report(self):
        ...
        for s in bhl_status:
            print(
                f"{s['module']:<20} | {s['language']:<6} | {s['status']:<15} | {s['size']}"
            )
        ...
        print(
            f"- 系统内存占用: {resources['memory_used']} / {resources['memory_total']} ({resources['memory_percent']}%)"
        )
    ...
    def run_self_healing(self):
        ...
        if not missing:
            ...
            print(
                "[提示] 您可以运行各模块目录下的 build.sh 进行手动编译，或使用 Butler 自动构建工具。"
            )
        ...
        if psutil:
            ...
            if mem_p > 90:
                print(
                    f"[危险] 系统内存占用过高 ({mem_p}%)！正在尝试触发资源保护策略..."
                )
