    ...
    def import_module(self, name, globals=None, locals=None, fromlist=(), level=0):
        ...
        if fromlist:
            for attr in fromlist:
                ...
                # 检查模块特定的属性限制
                if (
                    name in self.module_attributes
                    and attr not in self.module_attributes[name]
                ):
...
class ResourceMonitor(threading.Thread):
    ...
    def __init__(
        self, interval=0.1, memory_limit=100 * 1024 * 1024, instruction_limit=1000000
    ):
        super().__init__()
        ...
        while not self.stop_event.wait(self.interval):
            ...
            if current_memory > self.memory_limit:
                raise ResourceLimitError(
                    f"内存使用超过限制: {current_memory} > {self.memory_limit}"
                )
    ...
    def count_instruction(self):
        ...
        if self.instruction_count > self.instruction_limit:
            raise ResourceLimitError(
                f"指令执行超过限制: {self.instruction_count} > {self.instruction_limit}"
            )
...
class Sandbox:
    ...
    def __init__(
        self, timeout=30, memory_limit=100 * 1024 * 1024, instruction_limit=1000000
    ):
        ...
        self.resource_monitor = ResourceMonitor(
            memory_limit=memory_limit, instruction_limit=instruction_limit
        )
...
if __name__ == "__main__":
    # 创建沙盒实例
    sandbox = Sandbox(
        timeout=10, memory_limit=50 * 1024 * 1024, instruction_limit=500000
    )
