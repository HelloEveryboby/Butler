...
class AuthorityManager:
    ...
    DEFAULT_CONFIG_PATH = os.path.join(
        os.path.dirname(__file__), "authority_config.json"
    )
    DEFAULT_HASH_STORAGE_FILE = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "file_system", "file_hashes.json"
    )
    ...
    def log_audit(self, username: str, operation: str, status: str):
        ...
        log_entry = (
            f"[{timestamp}] 用户: {username}, 操作: {operation}, 状态: {status}\n"
        )
    ...
    def check_permission(self, username: str, required_level: int) -> bool:
        ...
        if self.is_session_valid(username):
            if user_level >= required_level:
                ...
                logger.warning(
                    f"用户 '{username}' 权限等级不足 (拥有: {user_level}, 需要: {required_level})"
                )
                print(
                    f"权限不足: 用户 {username} 的级别为 {user_level}，但此操作需要级别 {required_level}。"
                )
                return False
        ...
        if self.verify_user(username, password):
            if user_level >= required_level:
                ...
                logger.warning(
                    f"用户 '{username}' 验证成功但权限不足 (拥有: {user_level}, 需要: {required_level})"
                )
                print("权限不足")
                return False
        else:
...
def main():
    ...
    parser.add_argument(
        "--add-user",
        nargs=3,
        metavar=("USERNAME", "PASSWORD", "LEVEL"),
        help="添加或更新用户",
    )
    parser.add_argument(
        "--list-users", action="store_true", help="列出所有用户 (仅用户名)"
    )
            ...
            if authority_manager.users.get(current_user, {}).get("permission", 0) >= 3:
                try:
                    if os.path.exists(AuthorityManager.DEFAULT_AUDIT_LOG):
                        with open(
                            AuthorityManager.DEFAULT_AUDIT_LOG, "r", encoding="utf-8"
                        ) as f:
                            print("\n--- 最近 20 条审计日志 ---")
                            lines = f.readlines()
