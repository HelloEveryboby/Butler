...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("qr_scanner.log"), logging.StreamHandler()],
)
...
def select_save_folder(default_save_folder):
    ...
    folder = filedialog.askdirectory(
        initialdir=default_save_folder, title="选择保存文件夹"
    )
    root.destroy()
...
def detect_and_handle_qr_code(image, save_folder, source="camera"):
    ...
    try:
        ...
        if bbox is not None and data:
            ...
            # 在图像上显示URL
            cv2.putText(
                image, data, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
            )
...
class QRScannerApp:
    ...
    def __init__(self, root):
        ...
        ttk.Radiobutton(
            mode_frame, text="摄像头扫描", variable=self.scan_mode, value="camera"
        ).pack(side=tk.LEFT, padx=10)

        ttk.Radiobutton(
            mode_frame, text="屏幕扫描", variable=self.scan_mode, value="screen"
        ).pack(side=tk.LEFT, padx=10)
        ...
        ttk.Button(path_frame, text="更改", command=self.change_save_folder).pack(
            side=tk.RIGHT
        )
        ...
        ttk.Button(
            button_frame, text="开始扫描", command=self.start_scan, width=15
        ).pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(
            button_frame, text="退出程序", command=lambda: on_exit(root), width=15
        ).pack(side=tk.RIGHT)
    ...
    def change_save_folder(self):
        """更改保存文件夹"""
        folder = filedialog.askdirectory(
            initialdir=self.save_folder.get(), title="选择保存文件夹"
        )
        if folder:
