...
def export_to_excel(df, analysis, output_file):
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        if "PCA解释方差比" in analysis:
            pd.Series(analysis["PCA解释方差比"]).to_excel(
                writer, sheet_name="PCA解释方差比"
            )
...
def create_excel_charts(worksheet, df):
    ...
    for idx, column in enumerate(numeric_columns, start=1):
        ...
        if len(date_columns) > 0:
            ...
            dates = Reference(
                worksheet,
                min_col=df.columns.get_loc(date_columns[0]) + 1,
                min_row=2,
                max_row=len(df) + 1,
            )
    ...
    for idx, column in enumerate(
        categorical_columns[:3], start=1
    ):  # 只为前三个分类列创建饼图
        pie_chart = PieChart()
        pie_chart.title = f"{column}分布"
        labels = Reference(
            worksheet,
            min_col=df.columns.get_loc(column) + 1,
            min_row=2,
            max_row=len(df) + 1,
        )
        data = Reference(
    ...
    if len(numeric_columns) >= 2:
        ...
        xvalues = Reference(
            worksheet,
            min_col=df.columns.get_loc(numeric_columns[0]) + 1,
            min_row=2,
            max_row=len(df) + 1,
        )
        yvalues = Reference(
            worksheet,
            min_col=df.columns.get_loc(numeric_columns[1]) + 1,
            min_row=2,
            max_row=len(df) + 1,
        )
        ...
        worksheet.add_chart(
            scatter_chart,
            f"{chr(65 + len(numeric_columns) + len(categorical_columns) + 1)}1",
        )


def process_excel_files(
    directory, filter_column=None, filter_condition=None, merge_column=None
):
    """处理Excel文件的主函数"""
    print("正在加载Excel文件...")
...
def main():
    ...
    filter_condition = (
        input("请输入筛选条件（例如: >100, <50, =值, 包含文本）: ")
        if filter_column
        else None
    )
