...
def open_image_file(file_path):
    ...
    try:
        ...
        logger.info(f"使用系统默认查看器打开图片：{file_path}")
    except Exception as e:
        logger.error(f"打开图片文件 {file_path} 时出错：{e}")
...
def open_text_file(file_path):
    ...
    try:
        if platform.system() == "Windows":
            ...
            subprocess.run(["xdg-open", file_path])
        logger.info(f"使用系统默认编辑器打开文本文件：{file_path}")
    except Exception as e:
        logger.error(f"打开文本文件 {file_path} 时出错：{e}")
