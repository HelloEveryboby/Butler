    ...
    def decompress(archive_path, dest_dir):
        ...
        try:
            if zipfile.is_zipfile(archive_path):
                ...
                raise ValueError(
                    f"Unsupported or corrupted archive file: {archive_path}"
                )
...
def open_file(file_path):
    ...
    try:
        ...
        if ext in [".png", ".jpg", ".jpeg", ".bmp", ".gif"]:
            # For simplicity, we use the existing save_image_file as a placeholder
            # or ideally we would use a system opener.
            logger.info(f"正在尝试打开图片文件：{file_path}")
        elif ext in [".txt", ".json", ".log", ".xml"]:
            logger.info(f"正在尝试打开文本文件：{file_path}")
        else:
