...
def convert_file(input_file_path, output_file_path, output_folder):
    ...
    if from_fmt in ["MARKDOWN", "HTM"]:
        from_fmt = "MD" if "M" in from_fmt else "HTML"
    if to_fmt in ["MARKDOWN", "HTM"]:
        to_fmt = "MD" if "M" in to_fmt else "HTML"
    if from_fmt in ["JPEG", "JPG"]:
        from_fmt = "JPG"
    if to_fmt in ["JPEG", "JPG"]:
        to_fmt = "JPG"
    ...
    if (from_fmt, to_fmt) in supported_routes:
        ...
        target_path = (
            os.path.join(output_folder, os.path.basename(output_file_path)) if output_folder else output_file_path
        )
        from skills.format_convert.format_convert import handle_request

        kwargs = {"input": input_file_path, "from": from_fmt, "to": to_fmt, "save_to": target_path}
