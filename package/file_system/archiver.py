...
class ArchiverApp(tk.Toplevel):
    ...
    def build_compress_tab(self):
        ...
        list_frame = ttk.LabelFrame(
            self.compress_tab, text="Files and Folders to Compress"
        )
        ...
        ttk.Button(
            list_button_frame,
            text="Add File(s)",
            command=lambda: self.add_items("files"),
        ).pack(side="left", padx=5, pady=5)
        ttk.Button(
            list_button_frame,
            text="Add Directory",
            command=lambda: self.add_items("dirs"),
        ).pack(side="left", padx=5, pady=5)
        ttk.Button(
            list_button_frame, text="Remove Selected", command=self.remove_selected
        ).pack(side="left", padx=5, pady=5)
        ttk.Button(list_button_frame, text="Clear All", command=self.clear_list).pack(
        ...
        self.compress_button = ttk.Button(
            self.compress_tab, text="Compress", command=self.start_compression_thread
        )
    ...
    def build_extract_tab(self):
        ...
        ttk.Label(input_frame, text="Archive File:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(input_frame, textvariable=self.archive_path_var, width=60).grid(
            row=0, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(
            input_frame, text="Browse...", command=self.browse_archive_file
        ).grid(row=0, column=2, padx=5, pady=5)
        ...
        ttk.Label(dest_frame, text="Extract to:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(dest_frame, textvariable=self.dest_path_var, width=60).grid(
            row=0, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(dest_frame, text="Browse...", command=self.browse_dest_folder).grid(
        ...
        self.extract_button = ttk.Button(
            self.extract_tab, text="Extract", command=self.start_extraction_thread
        )
    ...
    def start_compression_thread(self):
        ...
        if not items_to_compress:
            messagebox.showwarning(
                "Warning", "No files or folders to compress.", parent=self
            )
            return
        if not output_path:
            messagebox.showwarning(
                "Warning", "Please specify an output file path.", parent=self
            )
            return
        ...
        thread = threading.Thread(
            target=self.compress_files, args=(items_to_compress, output_path)
        )
        thread.daemon = True
        thread.start()
    ...
    def compress_files(self, items, output_path):
        try:
                        ...
                        for root, _, files in os.walk(item):
                            for file in files:
                                ...
                                arcname = os.path.join(
                                    base_folder, os.path.relpath(file_path, item)
                                )
                                zipf.write(file_path, arcname)
                                files_processed += 1
                                self.update_progress(
                                    files_processed, f"Compressing: {file}"
                                )
            ...
            messagebox.showerror(
                "Error", f"An error occurred during compression:\n{e}", parent=self
            )
        finally:
    ...
    def start_extraction_thread(self):
        ...
        if not archive_path or not os.path.exists(archive_path):
            messagebox.showwarning(
                "Warning", "Please select a valid archive file.", parent=self
            )
            return
        if not dest_path or not os.path.isdir(dest_path):
            messagebox.showwarning(
                "Warning", "Please select a valid destination folder.", parent=self
            )
            return
        ...
        thread = threading.Thread(
            target=self.extract_files, args=(archive_path, dest_path)
        )
        thread.daemon = True
        thread.start()
    ...
    def extract_files(self, archive_path, dest_path):
        try:
            ...
            messagebox.showinfo(
                "Success",
                f"Successfully extracted archive to:\n{dest_path}",
                parent=self,
            )
