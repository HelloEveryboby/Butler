...
import datetime
import threading
from .agent import register_intent
from . import algorithms
from utils.document import task_manager_bridge # Register task_manage intent
...
def handle_open_switchboard(jarvis_app, **kwargs):
    """打开程序交换机以防止系统混乱。"""
    try:
        from utils import autonomous_switch
        # 启动后台守护进程
...
def handle_cleanup(jarvis_app, **kwargs):
    ...
    try:
        from utils import data_recycler
...
def handle_query_local_knowledge(jarvis_app, entities, **kwargs):
    ...
    try:
        from utils import knowledge_base_manager
...
def handle_cloud_upload(jarvis_app, entities, **kwargs):
    ...
    def run_upload():
        try:
            from utils import cloud_storage_manager
...
def handle_crawl(jarvis_app, entities, **kwargs):
    ...
    def run_crawl():
        try:
            from utils.network.crawler import run as crawl_run
...
def handle_email_op(jarvis_app, entities, **kwargs):
    ...
    def run_email():
        try:
            from utils.network.e_mail import EmailAssistant
...
def handle_image_search(jarvis_app, entities, **kwargs):
    ...
    def run_img_search():
        try:
            from utils.network.image_search_tool import run as img_run
...
def handle_crypto_op(jarvis_app, entities, **kwargs):
    ...
    def run_crypto():
        try:
            from utils.security.encrypt import SecureVault
...
def handle_get_weather(jarvis_app, entities, **kwargs):
    ...
    def run_weather():
        try:
            from utils.network.weather import get_weather_from_web
...
def handle_manage_file(jarvis_app, entities, **kwargs):
    ...
    try:
        from utils.file_system.file_manager import FileManager
        fm = FileManager()
...
def handle_convert_file(jarvis_app, entities, **kwargs):
    ...
    def run_convert():
        try:
            from utils.document.file_converter import run as conv_run
...
def handle_translate_op(jarvis_app, entities, **kwargs):
    ...
    def run_trans():
        try:
            from utils.document.translators import (
...
def handle_system_monitor(jarvis_app, **kwargs):
    """系统监控意图处理。"""
    try:
        from utils.core_utils.health_monitor import run as monitor_run
        monitor_run()
...
def handle_system_audit(jarvis_app, entities, **kwargs):
    ...
    try:
        from utils.core_utils.system_executor_tool import run as audit_run
...
def handle_grep_search(jarvis_app, entities, **kwargs):
    ...
    def run_grep():
        try:
            from utils.core_utils.dev_tools import dev_tools
