...
function setStatus(text, cls) {
    ...
    if (statusIndicator)
        statusIndicator.className = cls || 'status-idle';
}
function log(html) {
...
async function loadSoftware() {
    ...
    box.innerHTML = data.items
        .map((it) => `
        <div class="data-row" onclick="(document.getElementById('uninstall-name') as HTMLInputElement).value='${escapeHtml(it.name)}'">
            <div class="row-main">
...
async function doUninstall() {
    const nameInput = document.getElementById('uninstall-name');
    const name = nameInput?.value.trim() || '';
    if (!name) {
        alert('请输入软件名');
        return;
    }
    const dryRunCheckbox = document.getElementById('dry-run-uninstall');
    ...
    if (data.leftovers && data.leftovers.length) {
        detail +=
            '<div class="leftover-list">' +
                data.leftovers
                    .map((l) => `<div class="leftover-item"><span class="lk">${escapeHtml(l.kind)}</span> ${escapeHtml(l.path)} <span class="ls">${escapeHtml(l.size_text)}</span></div>`)
                    .join('') +
                '</div>';
    }
    log(detail);
...
async function cleanJunk() {
    const dryRunCheckbox = document.getElementById('dry-run-clean');
    const dryRun = dryRunCheckbox?.checked || false;
...
async function loadSnapshot() {
    ...
    if (data.disks && data.disks.length) {
        disksHtml = data.disks
            .map((d) => `<div class="metric-row"><span class="mk">磁盘 ${escapeHtml(d.mountpoint)}</span><span class="mv">${bar(d.percent)}</span><span class="ms">${d.used_text} / ${d.total_text}</span></div>`)
            .join('');
    }
    else {
        disksHtml =
    ...
    box.innerHTML = `
        <div class="snapshot">
            ...
            ${m.swap_total
        ? `<div class="metric-row"><span class="mk">Swap</span><span class="mv">${bar(m.swap_percent)}</span><span class="ms">${formatBytes(m.swap_used)} / ${formatBytes(m.swap_total)}</span></div>`
        : ''}
            ${disksHtml}
...
if (typeof window !== 'undefined') {
    window.callButlerSkill = callButlerSkill;
    ...
    window.doUninstall = doUninstall;
    window.scanJunk = scanJunk;
    window.cleanJunk = cleanJunk;
