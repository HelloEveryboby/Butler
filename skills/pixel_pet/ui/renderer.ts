...
export const PAL: Array<string | null> = [
  null,
  '#3D3D4A',
  '#F5C6A5',
  '#1A1A1A',
...
export class PixelGrid {
  ...
  public rect(x: number, y: number, w: number, h: number, c: number): void {
...
const GW = 18;
const GH = 30;
const MAP: Record<string, number> = { D: D, L: L, B: B, G: G, P: P, W: W };
...
function parseMap(mapData: string[]): PixelGrid {
...
function buildDog(mood: string): PixelGrid {
...
function prerenderGrid(grid: PixelGrid): HTMLCanvasElement {
...
const cache = new Map<PixelGrid, HTMLCanvasElement>();
function getCached(grid: PixelGrid): HTMLCanvasElement {
  if (!cache.has(grid)) cache.set(grid, prerenderGrid(grid));
  return cache.get(grid)!;
}

let lastFrame: PixelGrid | null = null;
function drawFrame(grid: PixelGrid): void {
  if (!ctx || !cv) return;
...
const MOOD_FRAMES: Record<string, PixelGrid> = {
  idle: frameIdle,
  happy: frameHappy,
  sad: frameSad,
  angry: frameAngry,
...
const MOOD_PARTICLES: Record<string, string[] | null> = {
  idle: null,
  happy: ['💕', '💗', '🐶', '✨'],
  sad: ['💧', '😢', '💦'],
  angry: ['💢', '⚡', '🔥'],
...
function setMood(mood: string, statusTextOverride: string | null = null): void {
  currentMood = mood;
  moodTimer = 300;
...
function spawnHearts(cx: number, cy: number, mood: string): void {
...
function setUIMode(mode: string): void {
  currentMode = mode;
  const body = document.body;
  if (mode === 'panel') {
    body.classList.remove('mode-widget');
    body.classList.add('mode-panel');
    if (window.pywebview && window.pywebview.api && (window.pywebview.api as any).toggle_mode) {
      (window.pywebview.api as any).toggle_mode(true);
    }
  } else {
    body.classList.remove('mode-panel');
    body.classList.add('mode-widget');
    if (window.pywebview && window.pywebview.api && (window.pywebview.api as any).toggle_mode) {
      (window.pywebview.api as any).toggle_mode(false);
    }
  }
}
...
if (typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const sz = document.getElementById('sz') as HTMLInputElement;
    sz?.addEventListener('input', (e: Event) => {
      px = +(e.target as HTMLInputElement).value;
      resize();
      invalidateCache();
    });
    ...
    document.querySelectorAll('.mood-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const mood = (btn as HTMLElement).dataset.mood || 'idle';
        setMood(mood);
        const rect = cv.getBoundingClientRect();
...
(window as any).ButlerPet = {
  onEvent: function (payload: { event: string; message?: string }) {
