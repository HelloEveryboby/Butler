// System Cleaner Pro — Butler 前端桥接逻辑 (TypeScript)
...
async function toggleMonitor(): Promise<void> {
  ...
  if (!isTracking) {
    if (text) text.innerText = '准备快照中...';
    const data = await callButlerSkill('start_track');
    console.log('start_track response:', data);
    if (data && data.status === 'tracking_started') {
      isTracking = true;
      if (btn) btn.innerText = '🛑 捕获系统变更并分析';
      if (indicator) indicator.className = 'status-tracking';
      if (text) text.innerText = '正在监视新软件安装...';
      if (logBody) logBody.innerHTML = '已生成初始快照。<br>请现在开始安装软件...';
    } else {
      if (text) text.innerText = '快照失败';
      if (logBody) logBody.innerHTML = `<span style="color:#f87171">错误: ${data ? data.message : 'Unknown'}</span>`;
    }
  } else {
    if (text) text.innerText = '分析差异中...';
    const data = await callButlerSkill('stop_track');
    console.log('stop_track response:', data);
    isTracking = false;
    if (btn) btn.innerText = '⚡ 开启安装监视';
    if (indicator) indicator.className = 'status-idle';

    if (data && data.status === 'error') {
      if (text) text.innerText = '分析失败';
      if (logBody) logBody.innerHTML = `<span style="color:#f87171">错误: ${data.message}</span>`;
    } else if (data) {
      if (text) text.innerText = '分析完成';
      if (logBody) {
        logBody.innerHTML = `
...
async function setupSilentMode(): Promise<void> {
  const logBody = document.getElementById('log-output');
  if (logBody) logBody.innerHTML = '正在请求一次性授权以开启静默清理通道...';
  const data = await callButlerSkill('setup_elevation');
  alert(data.message);
  if (logBody) logBody.innerHTML = data.message;
}

async function requestClean(): Promise<void> {
  const logBody = document.getElementById('log-output');
  if (logBody) logBody.innerHTML += '<br><span style="color: #fbbf24;">正在请求系统级提权授权...</span>';

  const data = await callButlerSkill('execute_clean');
  if (data && data.message) {
    alert(data.message);
    if (data.message.includes('成功')) {
      const cleanBtn = document.getElementById('btn-clean') as HTMLButtonElement;
      if (cleanBtn) cleanBtn.disabled = true;
      if (logBody) logBody.innerHTML += '<br><span style="color: #10b981;">清理执行完毕。</span>';
    }
  }
}
...
if (typeof window !== 'undefined') {
  ...
  (window as any).setupSilentMode = setupSilentMode;
  (window as any).requestClean = requestClean;
}
