...
*   **技能系统 (Skills)**：使用即插即用的 Skills 系统轻松扩展 Butler 的功能。
...
*   **技能系统 (Skills)**：Butler 的功能可以通过动态技能系统进行扩展。`SkillManager` 负责发现、加载和执行 `skills/` 目录中的技能。每个技能都是一个独立的模块，支持 Python 入口和二进制执行，并配有 AI 友好的 `SKILL.md` 指令集。
...
*   `skills/`：Butler 的核心扩展库。每个技能都是一个独立的目录，包含 `SKILL.md` 指令、Python 入口（`__init__.py`）或二进制文件。由 `SkillManager` 统一管理，支持即插即用和 AI 自动感知。
...
`skills/` 目录包含扩展 Butler 助手功能的技能。每个技能都是一个独立的模块，遵循 Butler 技能开发规范。

`SkillManager` 将自动加载并监控放置在此目录中的任何有效技能。
