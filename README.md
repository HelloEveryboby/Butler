...
Butler 助手建立在模块化且可扩展的架构之上，旨在实现灵活性和可伸缩性。其核心是 `Butler` 类，它充当中央编排器，管理信息流并协调系统的各个组件。

关键架构组件包括：

*   **命令处理**：通过支持多条执行路径的高级命令处理系统处理用户输入。简单的命令可以由 `Butler` 类直接处理，而更复杂的查询则路由到适当的子系统。
...
3.  **执行**：
    *   **意图处理**：如果匹配到意图，则调用 `Butler` 类中相应的处理程序。这可能涉及调用 `algorithms` 库中的函数、与插件交互或执行包。
    *   **插件执行**：如果命令旨在用于插件，`PluginManager` 将识别正确的插件并执行其 `run` 方法。
    *   **包执行**：如果命令对应于包，`Butler` 类将执行包的 `run()` 函数。
...
*   `butler/`：Butler 助手的核心。此目录包含主应用程序逻辑，包括编排整个系统的 `Butler` 类。它还包含基于 Tkinter 的 GUI (`CommandPanel.py`)、对话式 AI 集成以及算法库的 REST API。
...
Butler 现在支持 **零配置启动**。您无需手动安装依赖或配置环境，只需按照以下步骤操作：
...
2.  **直接运行：**
    *   **Windows**: 双击 `run.bat` (经典模式) 或 `run_modern.bat` (现代模式)。
    *   **Linux/macOS**: 运行 `./run.sh` 或 `./run_modern.sh`。

3.  **跟随引导：**
    系统会自动为您安装必要的本地依赖并弹出配置向导，您只需在界面中输入 API 密钥即可开始使用。详细流程请参考 [开箱即用工作流说明](docs/OOTB_WORKFLOW.md)。
