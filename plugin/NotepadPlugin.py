    ...
    def add_note(self, note: str) -> PluginResult:
        if not note:
            return PluginResult.new(
                ...
                need_call_brain=False,
                success=False,
                error_message="缺少笔记参数",
            )

        self.notes.append(note)
        return PluginResult.new(
            result="笔记添加成功", need_call_brain=False, success=True
        )

    def view_notes(self) -> PluginResult:
        if not self.notes:
            return PluginResult.new(
                result="没有可用的笔记", need_call_brain=False, success=True
            )
        return PluginResult.new(
    ...
    def edit_note(self, index: int, new_note: str) -> PluginResult:
        ...
        if not new_note:
            return PluginResult.new(
                ...
                need_call_brain=False,
                success=False,
                error_message="缺少新笔记内容",
            )

        self.notes[index] = new_note
        return PluginResult.new(
            result="笔记编辑成功", need_call_brain=False, success=True
        )
    ...
    def search_notes(self, keyword: str) -> PluginResult:
        if not keyword:
            return PluginResult.new(
                ...
                need_call_brain=False,
                success=False,
                error_message="缺少搜索关键字",
            )

        found_notes = [note for note in self.notes if keyword in note]
        if not found_notes:
            return PluginResult.new(
                result="没有找到匹配的笔记", need_call_brain=False, success=True
            )
        return PluginResult.new(
