...
class AddLongMemoryPlugin(AbstractPlugin):
    ...
    def run(self, takecommand: str, args: dict) -> PluginResult:
        ...
        # Access jarvis from the app context if needed.
        # Plugins usually get it via some injection or we can import it if it's a singleton.
        # Based on butler_app.py, we might need a better way, but for now let's fix the scope.
        from butler.butler_app import extension_manager

        jarvis = getattr(extension_manager, "jarvis_app", None)

        if takecommand is None and not args:
            self._logger.warning("没有检测到指令")
            return PluginResult.new("没有检测到指令", need_call_brain=False)

        content = args.get("content")
        if content and jarvis:
            try:
                ...
                return PluginResult.new(
                    result=f"已成功记忆：{content}", need_call_brain=True
                )
            except Exception as e:
                self._logger.error(f"记忆内容时出错: {str(e)}")
                return PluginResult.new(
                    result=f"记忆失败: {str(e)}", need_call_brain=False
                )
        else:
            self._logger.warning("记忆内容不能为空或 Jarvis 未就绪")
            return PluginResult.new(
                result="记忆内容不能为空或系统未就绪", need_call_brain=False
            )
