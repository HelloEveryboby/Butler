...
class SystemShieldPlugin(AbstractPlugin):
    ...
    def run(self, command: str, args: dict) -> PluginResult:
        ...
        # 3. 文件校验 (使用 Rust 模块: hybrid_crypto)
            ...
            path = args.get("path")
            if not path:
                return PluginResult.new(result=None, success=False, error_message="缺少 'path' 参数")
            return self._verify_file_integrity(path)

        return PluginResult.new(result=None, success=False, error_message=f"不支持的命令: {command}")

    def _get_system_status(self) -> PluginResult:
        client = self.get_hybrid_client("hybrid_sysutil")
        if not client:
            return PluginResult.new(result=None, success=False, error_message="无法加载 C 语言高性能系统工具模块")
    ...
    def _run_security_scan(self, target: str) -> PluginResult:
        ...
        if not client:
            return PluginResult.new(result=None, success=False, error_message="无法加载 Go 语言高并发网络模块")
        ...
        if not scan_result:
            return PluginResult.new(result=None, success=False, error_message="扫描失败")
    ...
    def _verify_file_integrity(self, path: str) -> PluginResult:
        if not os.path.exists(path):
            return PluginResult.new(result=None, success=False, error_message=f"文件不存在: {path}")

        client = self.get_hybrid_client("hybrid_crypto")
        if not client:
            return PluginResult.new(result=None, success=False, error_message="无法加载 Rust 语言高性能加密模块")
        ...
        if hash_result and "hash" in hash_result:
            ...
            return PluginResult.new(result=f"文件 SHA256 校验值: {file_hash}", additional_data={"path": path, "hash": file_hash})
        else:
            return PluginResult.new(result=None, success=False, error_message="哈希计算失败")
