...
class ArchiveManagerPlugin(AbstractPlugin):
    def __init__(self):
        ...
        self.tracked_files: Dict[
            str, Dict
        ] = {}  # {extracted_path: {zip_path, file_in_zip, initial_hash}}
        ...
        if command == "open_zip_file":
            ...
            if not zip_path or not file_in_zip:
                return PluginResult.new(
                    None, success=False, error_message="Missing zip_path or file_in_zip"
                )
            ...
            if not extracted_path:
                return PluginResult.new(
                    None, success=False, error_message="Missing extracted_path"
                )
            ...
            if not zip_path:
                return PluginResult.new(
                    None, success=False, error_message="Missing zip_path"
                )
            return self.list_contents(zip_path)

        return PluginResult.new(
            None, success=False, error_message=f"Unknown command: {command}"
        )
    ...
    def list_contents(self, zip_path: str) -> PluginResult:
        if not os.path.exists(zip_path):
            return PluginResult.new(
                None, success=False, error_message="Zip file not found"
            )
        try:
    ...
    def open_and_track(self, zip_path: str, file_in_zip: str) -> PluginResult:
        ...
        try:
            ...
            return PluginResult.new(
                {"extracted_path": extracted_path}, status=f"Monitoring {file_in_zip}"
            )
        except Exception as e:
            return PluginResult.new(None, success=False, error_message=str(e))
    ...
    def apply_sync(self, extracted_path: str, action: str) -> PluginResult:
        if extracted_path not in self.tracked_files:
            return PluginResult.new(
                None, success=False, error_message="File not tracked"
            )
    ...
    def _safe_replace_in_zip(
        self, zip_path: str, file_in_zip: str, new_file_path: str
    ) -> PluginResult:
            ...
            with zipfile.ZipFile(zip_path, "r") as zin:
                with zipfile.ZipFile(
                    ...
                    for i, item in enumerate(infolist):
                        ...
                        event_bus.emit(
                            "ui_output",
                            json.dumps({"type": "progress", "value": progress}),
                            "status_update",
                            None,
                        )
                    ...
                    event_bus.emit(
                        "ui_output",
                        json.dumps({"type": "progress", "value": 100}),
                        "status_update",
                        None,
                    )
