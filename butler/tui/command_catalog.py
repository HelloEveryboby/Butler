...
COMMANDS: list[CommandEntry] = [
    ...
    CommandEntry(
        ...
        category="⚙️ 系统",
        description="列出所有已加载的技能 (区分手动/Agent)",
        usage="/skills_list [user|agent|all]",
        detail="显示当前已加载的所有 Butler 技能。\n可按类型过滤: user=可调用技能, agent=Agent技能, all=全部。\n不带参数默认全部。",
        tags=["技能", "skill", "列表", "插件", "plugin", "agent", "手动"],
        example="/skills_list user",
    ),
    CommandEntry(
        name="skill_run", aliases=["sr", "运行技能"],
        category="⚙️ 系统",
        description="运行指定的可调用技能",
        usage="/skill_run <技能ID> [action] [--key value]",
        detail="手动运行一个可调用技能。仅适用于有 Python 入口的技能。\nAgent 技能无法直接运行，只能通过 AI 对话使用。",
        tags=["技能", "skill", "运行", "run", "执行"],
        example="/skill_run butler_expert ask --query 架构",
    ),
    CommandEntry(
        name="skill_info", aliases=["si", "技能详情"],
        category="⚙️ 系统",
        description="查看技能详情 (含类型标签)",
        usage="/skill_info <技能ID>",
        detail="显示技能的完整信息，包括类型标签 (🐍 可调用 / 📖 Agent)、描述、动作等。",
        tags=["技能", "详情", "info", "信息"],
    ...
    # ── 技能控制 (停止/状态/子操作) ──
    CommandEntry(
        name="skill_stop", aliases=["skillstop", "skstop", "停止技能"],
        category="🧰 技能",
        description="停止正在运行的后台技能服务",
        usage="/skill_stop <技能名>",
        detail="停止指定技能的后台服务。\n支持的技能: clip_magic, pixel_pet, focus",
        tags=["停止", "stop", "技能", "关闭", "kill"],
        example="/skill_stop clip_magic",
    ),
    CommandEntry(
        name="skill_status", aliases=["skstat", "技能状态", "技能状态"],
        category="🧰 技能",
        description="查看技能服务运行状态",
        usage="/skill_status [技能名]",
        detail="查看指定技能的运行状态，或列出所有后台技能状态。\n不带参数列出全部。",
        tags=["状态", "status", "技能", "查询"],
        example="/skill_status clip_magic",
    ),
    CommandEntry(
        name="clip_history", aliases=["cliphist", "剪贴板历史"],
        category="🧰 技能",
        description="查看剪贴板分类历史记录",
        usage="/clip_history",
        detail="查看 ClipMagic 最近分类的剪贴板内容。\n需要先通过 /clip_magic 启动服务。",
        tags=["剪贴板", "历史", "clipboard", "history", "记录"],
        example="/clip_history",
    ),
    CommandEntry(
        name="uninstall_scan", aliases=["uscan", "残留扫描", "扫描残留"],
        category="🧰 技能",
        description="扫描软件卸载后的残留文件",
        usage="/uninstall_scan <软件名>",
        detail="扫描指定软件卸载后残留的注册表项和文件。",
        tags=["扫描", "残留", "uninstall", "scan", "leftovers"],
        example="/uninstall_scan firefox",
    ),
    CommandEntry(
        name="uninstall_do", aliases=["udo", "深度卸载", "执行卸载"],
        category="🧰 技能",
        description="深度卸载指定软件 (调用卸载程序+清理残留)",
        usage="/uninstall_do <软件名> [| dry_run]",
        detail="深度卸载: 调用官方卸载程序 → 扫描残留 → 清理残留。\ndry_run=true 仅模拟不实际删除。\n默认 dry_run=true。",
        tags=["卸载", "uninstall", "深度", "清理"],
        example="/uninstall_do firefox | dry_run",
    ),
    CommandEntry(
        name="junk_scan", aliases=["jscan", "垃圾扫描", "扫描垃圾"],
        category="🧰 技能",
        description="扫描系统垃圾文件",
        usage="/junk_scan [| categories]",
        detail="扫描系统中的垃圾文件（缓存/临时文件/日志等）。\n可选 categories 过滤类型。",
        tags=["垃圾", "扫描", "junk", "scan", "清理"],
        example="/junk_scan",
    ),
    CommandEntry(
        name="junk_clean", aliases=["jclean", "清理垃圾", "执行清理"],
        category="🧰 技能",
        description="清理系统垃圾文件",
        usage="/junk_clean [| dry_run]",
        detail="清理扫描到的垃圾文件。\ndry_run=true 仅模拟，默认 true。\ndry_run=false 实际删除。",
        tags=["垃圾", "清理", "clean", "junk", "删除"],
        example="/junk_clean | dry_run=false",
    ),
    CommandEntry(
        name="sys_info", aliases=["sinfo", "系统信息", "硬件信息"],
        category="🧰 技能",
        description="查看系统硬件和资源信息",
        usage="/sys_info",
        detail="显示 CPU、内存、磁盘、网络的详细信息和实时使用率。",
        tags=["系统", "信息", "system", "info", "cpu", "内存", "硬件"],
        example="/sys_info",
    ),
    CommandEntry(
        name="top_procs", aliases=["top", "进程列表", "top进程"],
        category="🧰 技能",
        description="查看占用最高的进程",
        usage="/top_procs [| cpu|memory]",
        detail="显示 CPU 或内存占用最高的进程列表。\n默认按 CPU 排序。",
        tags=["进程", "top", "cpu", "memory", "占用"],
        example="/top_procs | memory",
    ),
    CommandEntry(
        name="cloud_list", aliases=["clist", "云盘文件", "列出云盘"],
        category="🧰 技能",
        description="列出云盘文件",
        usage="/cloud_list <云盘ID> [| 路径]",
        detail="列出指定云盘的文件和目录。\n路径默认为根目录 /。",
        tags=["云盘", "列出", "list", "files", "cloud"],
        example="/cloud_list onedrive_1 | /Documents",
    ),
    CommandEntry(
        name="cloud_search", aliases=["csearch", "搜索云盘", "云盘搜索"],
        category="🧰 技能",
        description="跨云盘搜索文件",
        usage="/cloud_search <关键词>",
        detail="在所有已配置的云盘中搜索文件名包含关键词的文件。",
        tags=["云盘", "搜索", "search", "find", "文件"],
        example="/cloud_search 报告",
    ),
    CommandEntry(
        name="cloud_transfer", aliases=["ctransfer", "云盘传输", "跨盘传输"],
        category="🧰 技能",
        description="跨云盘传输文件",
        usage="/cloud_transfer <源盘> | <目标盘> | <文件名> [| 源路径] [| 目标路径]",
        detail="在两个云盘之间传输文件，使用 RAM-Pipe 极速内存流管道。",
        tags=["云盘", "传输", "transfer", "复制", "跨盘"],
        example="/cloud_transfer onedrive_1 | webdav_1 | report.pdf",
    ),
    CommandEntry(
        name="cloud_status", aliases=["cstatus", "传输状态"],
        category="🧰 技能",
        description="查看云盘传输任务状态",
        usage="/cloud_status <任务ID>",
        detail="查看指定传输任务的进度、速度和状态。",
        tags=["云盘", "传输", "状态", "status", "进度"],
        example="/cloud_status task_1234567890",
    ),
    CommandEntry(
        name="cloud_duplicates", aliases=["cdup", "查重", "重复文件"],
        category="🧰 技能",
        description="查找云盘中的重复文件",
        usage="/cloud_duplicates",
        detail="扫描所有云盘，找出同名同大小的重复文件。",
        tags=["云盘", "重复", "duplicate", "查重"],
        example="/cloud_duplicates",
    ),
    CommandEntry(
        name="track_start", aliases=["tstart", "开始追踪", "安装追踪"],
        category="🧰 技能",
        description="开始追踪安装变更 (安装前快照)",
        usage="/track_start",
        detail="在安装软件前执行，捕获系统当前状态快照。\n安装完成后用 /track_stop 生成差异。",
        tags=["追踪", "track", "快照", "安装", "变更"],
        example="/track_start",
    ),
    CommandEntry(
        name="track_stop", aliases=["tstop", "停止追踪", "安装差异"],
        category="🧰 技能",
        description="停止追踪并生成安装差异报告",
        usage="/track_stop",
        detail="在安装软件后执行，捕获安装后快照并生成差异 Diff 矩阵。\n之后可用 /track_clean 清理。",
        tags=["追踪", "停止", "track", "差异", "diff"],
        example="/track_stop",
    ),
    CommandEntry(
        name="track_clean", aliases=["tclean", "执行清理", "强力清理"],
        category="🧰 技能",
        description="执行残留清理 (清除追踪到的变更)",
        usage="/track_clean",
        detail="根据 /track_stop 生成的差异报告，清理安装残留。\n需要先执行 track_start → 安装软件 → track_stop。",
        tags=["清理", "clean", "追踪", "残留", "删除"],
        example="/track_clean",
    ),
]
