...
class Jarvis:
    ...
    def _execute_with_llm_interpreter(self, command):
        ...
        for i in range(max_iterations):
            # Use NLU service to generate code
            prompt = f"{system_prompt}\n\nUser Question: {current_command}"
            ...
            if code_match:
                ...
                self.ui_print(f"Iteration {i+1}: AI 编写了以下代码：", tag='system_message')
                self._execute_with_interpreter("python", code)

                # In a real loop, we would capture the output and check if the user is satisfied
                # or if the AI needs to fix an error. For now, we finish after one successful block.
                break
            else:
