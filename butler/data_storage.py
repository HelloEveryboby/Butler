...
class DataStorageManager:
    ...
    def __init__(self):
        ...
        self.local_storage_path = (
            Path(__file__).resolve().parent.parent / "data" / "local_storage"
        )
        self.local_storage_path.mkdir(parents=True, exist_ok=True)

        if not self.redis_client:
            self._logger.warning(
                "DataStorageManager initialized without a Redis connection. Falling back to local file storage."
            )
        else:
    ...
    def save(self, plugin_name: str, key: str, value: Any):
        ...
        if self.redis_client:
            try:
                ...
                self._logger.info(
                    f"Saved data to Redis for plugin '{plugin_name}' with key '{key}'."
                )
                return
            except Exception as e:
                self._logger.error(
                    f"Failed to save data to Redis for plugin '{plugin_name}' with key '{key}': {e}"
                )
        ...
        try:
            ...
            self._logger.info(
                f"Saved data to local file for plugin '{plugin_name}' with key '{key}'."
            )
        except Exception as e:
            self._logger.error(
                f"Failed to save data locally for plugin '{plugin_name}' with key '{key}': {e}"
            )
    ...
    def load(self, plugin_name: str, key: str) -> Optional[Any]:
        ...
        if self.redis_client:
            try:
                ...
                if serialized_value:
                    self._logger.info(
                        f"Loaded data from Redis for plugin '{plugin_name}' with key '{key}'."
                    )
                    return json.loads(serialized_value)
            except Exception as e:
                self._logger.error(
                    f"Failed to load data from Redis for plugin '{plugin_name}' with key '{key}': {e}"
                )
        ...
        try:
            ...
            if local_path.exists():
                ...
                self._logger.info(
                    f"Loaded data from local file for plugin '{plugin_name}' with key '{key}'."
                )
                return json.loads(serialized_value)
            return None
        except Exception as e:
            self._logger.error(
                f"Failed to load data locally for plugin '{plugin_name}' with key '{key}': {e}"
            )
            return None
    ...
    def delete(self, plugin_name: str, key: str):
        ...
        if self.redis_client:
            try:
                ...
                self._logger.info(
                    f"Deleted data from Redis for plugin '{plugin_name}' with key '{key}'."
                )
                deleted = True
            except Exception as e:
                self._logger.error(
                    f"Failed to delete data from Redis for plugin '{plugin_name}' with key '{key}': {e}"
                )
        ...
        try:
            ...
            if local_path.exists():
                local_path.unlink()
                self._logger.info(
                    f"Deleted data from local file for plugin '{plugin_name}' with key '{key}'."
                )
                deleted = True
        except Exception as e:
            self._logger.error(
                f"Failed to delete data locally for plugin '{plugin_name}' with key '{key}': {e}"
            )

        if not deleted and not self.redis_client:
            self._logger.warning(
                f"No data found to delete for plugin '{plugin_name}' with key '{key}'."
            )
