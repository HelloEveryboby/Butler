...
from package.document import task_manager_bridge # Required for intent registration side effects
...
def handle_query_local_knowledge(jarvis_app, entities, **kwargs):
    ...
    try:
        ...
        results = knowledge_base_manager.run(
            operation="query", query=query, kb_name=kb_name
        )
...
def handle_index_local_files(jarvis_app, entities, **kwargs):
    ...
    def run_index():
        try:
            ...
            result = knowledge_base_manager.run(
                operation=operation, kb_name=kb_name, **{param: path}
            )
...
def handle_cloud_upload(jarvis_app, entities, **kwargs):
    ...
    def run_upload():
        try:
            ...
            result = cloud_storage_manager.run(
                operation="upload", local_path=local_path, remote_path=remote_path
            )
            jarvis_app.speak(result)
        except Exception as e:
...
def handle_cloud_download(jarvis_app, entities, **kwargs):
    ...
    def run_download():
        try:
            ...
            result = cloud_storage_manager.run(
                operation="download", remote_path=remote_path, local_path=local_path
            )
            jarvis_app.speak(result)
        except Exception as e:
...
def handle_marker_approve(jarvis_app, **kwargs):
    ...
    handle_marker_convert(
        jarvis_app,
        entities={"path": task["path"], "format": task["format"], "confirm": True},
    )
...
def handle_structured_extract(jarvis_app, entities, **kwargs):
    ...
    def run_extract():
        try:
            ...
            result = marker_tool.MarkerTool().convert(
                file_path=path, json_schema=schema
            )
...
def handle_crawl(jarvis_app, entities, **kwargs):
    ...
    def run_crawl():
        try:
            ...
            jarvis_app.ui_print(
                f"🚀 正在启动爬虫 (URL: {url or '搜索'}, 类型: {res_type})..."
            )
...
def handle_crypto_op(jarvis_app, entities, **kwargs):
    ...
    def run_crypto():
        try:
            ...
            jarvis_app.ui_print(
                f"🔐 正在执行{algo}{'加密' if op == 'encrypt' else '解密'}: {path}..."
            )
