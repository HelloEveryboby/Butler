...
# `butler_app` 参数是一个特殊情况，由分发器注入，以提供
# 对主应用程序实例的访问（用于 `speak` 和 `ui_print` 等方法）。

@register_intent("sort_numbers")
def handle_sort_numbers(butler_app, entities, **kwargs):
    """对 'numbers' 实体中提供的数字列表进行排序。"""
    try:
        numbers = entities.get("numbers", [])
        if not numbers or not all(isinstance(n, (int, float)) for n in numbers):
             butler_app.speak("排序失败，请提供有效的数字列表。")
             return
        sorted_nums = algorithms.quick_sort(numbers)
        butler_app.speak(f"排序结果: {sorted_nums}")
    except Exception as e:
        butler_app.speak(f"排序时发生错误: {e}")

@register_intent("find_number")
def handle_find_number(butler_app, entities, **kwargs):
    """在已排序的数字列表中查找目标数字的索引。"""
    try:
        numbers = entities.get("numbers", [])
        target = entities.get("target")
        if not numbers or target is None:
            butler_app.speak("查找失败，请提供数字列表和目标数字。")
            return

        numbers.sort()
        index = algorithms.binary_search(numbers, target)
        if index != -1:
            butler_app.speak(f"数字 {target} 在排序后的位置是: {index}")
        else:
            butler_app.speak(f"数字 {target} 不在数组中")
    except Exception as e:
        butler_app.speak(f"查找时发生错误: {e}")

@register_intent("calculate_fibonacci")
def handle_calculate_fibonacci(butler_app, entities, **kwargs):
    """计算斐波那契数列中的第 N 个数字。"""
    try:
        n = entities.get("number")
        if n is None or not isinstance(n, int):
            butler_app.speak("计算失败，请输入一个有效的整数。")
            return
        fib = algorithms.fibonacci(n)
        butler_app.speak(f"斐波那契数列第{n}项是: {fib}")
    except Exception as e:
        butler_app.speak(f"计算斐波那契数时出错: {e}")

@register_intent("edge_detect_image")
def handle_edge_detect_image(butler_app, entities, **kwargs):
    """检测给定文件路径图像中的边缘并保存结果。"""
    try:
        image_path = entities.get("path")
        if not image_path or not isinstance(image_path, str):
            butler_app.speak("图像处理失败，请提供有效的路径。")
            return
        ...
        if os.path.exists(image_path):
            ...
            if edges is not None:
                ...
                butler_app.speak(f"边缘检测完成，结果已保存到: {output_path}")
            else:
                butler_app.speak("图像处理失败，无法读取图片。")
        else:
            butler_app.speak("找不到指定的图像文件。")
    except Exception as e:
        butler_app.speak(f"图像处理时出错: {e}")

@register_intent("text_similarity")
def handle_text_similarity(butler_app, entities, **kwargs):
    """计算两段文本之间的余弦相似度分数。"""
    try:
        text1 = entities.get("text1")
        text2 = entities.get("text2")
        if not text1 or not text2:
            butler_app.speak("相似度计算失败，请提供两段文本。")
            return
        similarity = algorithms.text_cosine_similarity(text1, text2)
        butler_app.speak(f"文本相似度是: {similarity:.2f}")
    except Exception as e:
        butler_app.speak(f"计算相似度时出错: {e}")

@register_intent("open_program")
def handle_open_program(butler_app, entities, programs, **kwargs):
    """打开按名称指定的程序或应用程序。"""
    program_name = entities.get("program_name")
    if not program_name:
        butler_app.speak("无法打开程序，未指定程序名称。")
        return

    # This function relies on the `execute_program` method of the Butler instance
    # and the program mapping, so we delegate back to it.
    butler_app._handle_open_program(entities, programs)

@register_intent("open_switchboard", requires_entities=False)
def handle_open_switchboard(butler_app, **kwargs):
    """打开程序交换机以防止系统混乱。"""
    try:
        from package import autonomous_switch
        # 启动后台守护进程
        threading.Thread(target=autonomous_switch.run, daemon=True).start()
        butler_app.speak("自动交换机已在后台启动，将自动管理程序排序并防止混乱。")
    except Exception as e:
        butler_app.speak(f"无法启动自动交换机: {e}")

@register_intent("exit", requires_entities=False)
def handle_exit(butler_app, **kwargs):
    """退出 Butler 助手应用程序。"""
    butler_app._handle_exit()

@register_intent("get_current_time", requires_entities=False)
def handle_get_current_time(butler_app, **kwargs):
    """获取当前时间并播报。"""
    current_time = datetime.datetime.now().strftime("%H:%M")
    butler_app.speak(f"现在时间是 {current_time}")

@register_intent("cleanup", requires_entities=False)
def handle_cleanup(butler_app, **kwargs):
    """执行数据回收/清理系统以删除临时文件。"""
    butler_app.ui_print("正在执行系统数据回收...")
    try:
        from package import data_recycler
        summary = data_recycler.run()
        butler_app.speak(f"数据回收完成。{summary}")
    except Exception as e:
        butler_app.speak(f"数据回收失败: {e}")

@register_intent("query_local_knowledge")
def handle_query_local_knowledge(butler_app, entities, **kwargs):
    """在本地知识库中搜索相关信息。"""
    query = entities.get("query")
    kb_name = entities.get("kb_name", "default")
    if not query:
        butler_app.speak("请提供您想在知识库中查询的内容。")
        return

    try:
        from package import knowledge_base_manager
        results = knowledge_base_manager.run(operation="query", query=query, kb_name=kb_name)
        butler_app.ui_print(results)
        butler_app.speak(f"已在知识库 '{kb_name}' 中完成搜索，结果已显示在面板上。")
    except Exception as e:
        butler_app.speak(f"查询知识库时出错: {e}")

@register_intent("index_local_files")
def handle_index_local_files(butler_app, entities, **kwargs):
    """将本地文件或目录索引到知识库以供后续查询。"""
    path = entities.get("path")
    kb_name = entities.get("kb_name", "default")
    if not path:
        butler_app.speak("请提供要索引的文件或目录路径。")
        return

    if not os.path.exists(path):
        butler_app.speak(f"找不到路径: {path}")
        return
...
def handle_cloud_list(butler_app, entities, **kwargs):
    ...
    def run_list():
        try:
            from package import cloud_storage_manager
            butler_app.ui_print(f"正在获取网盘目录 {path} 的列表...")
            result = cloud_storage_manager.run(operation="list", path=path)
            if len(result) > 200:
                butler_app.ui_print(result)
                butler_app.speak(f"已获取目录 {path} 的列表，文件较多，请在面板查看。")
            else:
                butler_app.speak(result)
        except Exception as e:
            butler_app.speak(f"获取网盘列表时出错: {e}")

    threading.Thread(target=run_list, daemon=True).start()

@register_intent("cloud_info", requires_entities=False)
def handle_cloud_info(butler_app, **kwargs):
    """查看网盘的配额和空间使用信息。"""
    def run_info():
        try:
            from package import cloud_storage_manager
            butler_app.ui_print("正在查询网盘容量信息...")
            result = cloud_storage_manager.run(operation="info")
            butler_app.speak(result)
        except Exception as e:
            butler_app.speak(f"查询网盘信息时出错: {e}")

    threading.Thread(target=run_info, daemon=True).start()

@register_intent("manage_dependencies")
def handle_manage_dependencies(butler_app, entities, **kwargs):
    ...
    def run_dep_mgr():
        try:
            ...
            butler_app.ui_print(f"正在启动依赖管理器 (命令: {command})...")
            result = dependency_manager.run(command=command, package=package)
            butler_app.speak(result)
        except Exception as e:
            butler_app.speak(f"依赖管理执行失败: {e}")
...
def handle_marker_convert(butler_app, entities, **kwargs):
    ...
    if not confirm_bypass:
        ...
        try:
            ...
            butler_app.ui_print(f"--- 预解析完成 ---\n文件: {path}\n字数: {char_count}\n图像: {img_count}")
            butler_app.speak(f"文件预解析已完成。提取了约 {char_count} 个字符和 {img_count} 张图像。请确认是否继续调用 DeepSeek 进行精准转换？如果是，请说“确认转换”。")
            butler_app._last_marker_task_id = task_id # 记录在 app 实例中
            return
        except Exception as e:
            butler_app.speak(f"预解析失败: {e}")
            return
...
def handle_memory_record(butler_app, entities, **kwargs):
    """记录一条新的记忆。"""
    content = entities.get("content")
    mem_type = entities.get("type", "daily")
    if not content:
        butler_app.speak("请提供要记录的内容。")
        return
    from package.document.memory_tools import memory_tools
...
def handle_pdf_assistant(butler_app, entities, **kwargs):
    """切换到 PDF 助手模式或处理 PDF 相关任务。"""
    butler_app.ui_print("--- PDF 助手模式已激活 ---", tag='system_message')

    # 如果用户没有提供具体 PDF 路径，先询问
    path = entities.get("path")
    if not path:
        butler_app.speak("您好！我是您的 PDF 助手。请提供您需要处理的 PDF 文件路径，并告诉我您想进行的操作（如总结、提取表格或问答）。")
    else:
        # 如果提供了路径，直接触发解释器模式，并注入 PDF 助手上下文
        butler_app.ui_print(f"正在为您分析 PDF 文档: {path}")
        # 这里我们可以直接复用解释器逻辑，但可以在 prompt 中增强 PDF 助手的角色感
        butler_app._execute_with_llm_interpreter(f"请作为 PDF 助手，分析并处理以下文件：{path}。用户需求：{entities.get('operation', '解析该文档')}")

@register_intent("crawl")
def handle_crawl(butler_app, entities, **kwargs):
    """网页爬虫意图处理。"""
    ...
    def run_crawl():
        try:
            ...
            butler_app.ui_print(f"🚀 正在启动爬虫 (URL: {url or '搜索'}, 类型: {res_type})...")
            crawl_run(url=url, search_query=query, type=res_type)
            butler_app.speak("爬虫任务执行完毕。")
        except Exception as e:
            butler_app.speak(f"爬虫执行失败: {e}")

    threading.Thread(target=run_crawl, daemon=True).start()

@register_intent("email_op")
def handle_email_op(butler_app, entities, **kwargs):
    """邮件操作意图处理。"""
    ...
    def run_email():
        try:
            ...
            if op == "send":
                to = entities.get("to")
                subject = entities.get("subject", "来自 Butler 的邮件")
                body = entities.get("body", "")
                if to and body:
                    butler_app.ui_print(f"📧 正在发送邮件到 {to}...")
                    # 注意：send_email 内部 plan_step_complete 有 input() 确认，在某些 UI 下可能阻塞
                    # 这里假设是简单的后端调用或 UI 已处理确认
                    assistant.send_email(subject, body, to)
                    butler_app.speak("邮件已发送。")
                else:
                    butler_app.speak("发送邮件需要收件人和正文。")
            else:
                butler_app.ui_print("📥 正在检查未读邮件...")
                emails = assistant.fetch_unread_emails()
                assistant.display_emails(emails)
                butler_app.speak(f"已获取 {len(emails)} 封邮件，请在面板查看详情。")
        except Exception as e:
            butler_app.speak(f"邮件操作失败: {e}")

    threading.Thread(target=run_email, daemon=True).start()

@register_intent("image_search")
def handle_image_search(butler_app, entities, **kwargs):
    """图片搜索意图处理。"""
    query = entities.get("query")
    path = entities.get("path")
    ...
    def run_img_search():
        try:
            ...
            butler_app.ui_print(f"🔍 正在搜索图片: {query or path}...")
            img_run(query=query, path=path, mode=mode)
            butler_app.speak("图片搜索完成。")
        except Exception as e:
            butler_app.speak(f"图搜失败: {e}")

    threading.Thread(target=run_img_search, daemon=True).start()

@register_intent("crypto_op")
def handle_crypto_op(butler_app, entities, **kwargs):
    """加解密操作意图处理 (高级双重加密模式)。"""
    op = entities.get("operation", "encrypt")
    path = entities.get("path")

    if not path:
        butler_app.speak("请提供文件路径。")
        return
    ...
    def run_crypto():
        try:
            ...
            if not core_code:
                butler_app.speak("核心码未加载。请先设置您的 6 位核心码。")
                return

            butler_app.ui_print(f"🔐 正在执行高级双重{'加密' if op=='encrypt' else '解密'}: {path}...")
            if op == 'encrypt':
                out = butler_app.dual_encryptor.encrypt_file(path, core_code)
            else:
                out = butler_app.dual_encryptor.decrypt_file(path, core_code)
            butler_app.speak(f"文件处理完成: {out}")
        except Exception as e:
            butler_app.speak(f"加密操作失败: {e}")

    threading.Thread(target=run_crypto, daemon=True).start()

@register_intent("get_weather")
def handle_get_weather(butler_app, entities, **kwargs):
    """天气查询意图处理。"""
    city = entities.get("city")
    if not city:
        butler_app.speak("请告诉我你想查询哪个城市的天气。")
        return
    ...
    def run_weather():
        try:
            ...
            if res:
                ...
                butler_app.speak(report)
            else:
                butler_app.speak(f"抱歉，我没能查到{city}的天气。")
        except Exception as e:
            butler_app.speak(f"天气查询出错: {e}")

    threading.Thread(target=run_weather, daemon=True).start()

@register_intent("manage_file")
def handle_manage_file(butler_app, entities, **kwargs):
    """基础文件操作意图处理。"""
    ...
    if not op or not path:
        butler_app.speak("文件操作需要指定操作类型和路径。")
        return
    ...
    try:
        ...
        if op == "create" or op == "write":
            ...
            if success:
                butler_app.ui_print(f"📄 文件内容 ({path}):\n{res}")
                butler_app.speak("文件读取完成，内容已显示在面板。")
            else:
                butler_app.speak(res)
        elif op == "delete":
            success, msg = fm.delete_file(path)
            butler_app.speak(msg)
    except Exception as e:
        butler_app.speak(f"文件操作失败: {e}")

@register_intent("convert_file")
def handle_convert_file(butler_app, entities, **kwargs):
    """文件转换意图处理。"""
    input_p = entities.get("input_path")
    output_p = entities.get("output_path")

    if not input_p or not output_p:
        butler_app.speak("请提供输入和输出文件路径。")
        return

    def run_convert():
        try:
            from package.document.file_converter import run as conv_run
            butler_app.ui_print(f"🔄 正在转换文件: {input_p} -> {output_p}")
            conv_run(input_file=input_p, output_file=output_p)
            butler_app.speak("文件转换完成。")
        except Exception as e:
            butler_app.speak(f"转换失败: {e}")
...
def handle_translate_op(butler_app, entities, **kwargs):
    """翻译操作意图处理。"""
        ...
        try:
            ...
            if text:
                ...
                if len(text) > 50:
                    ...
                    butler_app.ui_print(json.dumps({
                        "type": "translation",
                    ...
                    butler_app.ui_print(f"🌐 翻译结果: {res}")
                butler_app.speak("翻译完成。")
            elif path:
                ...
                butler_app.speak(f"文件翻译完成，结果保存在 {out}")
            elif url:
                butler_app.ui_print(f"🚀 正在分析并翻译网页: {url}")
...
def handle_system_audit(butler_app, entities, **kwargs):
    ...
    try:
        ...
        butler_app.speak("高性能系统审计已完成。")
    except Exception as e:
        butler_app.speak(f"审计运行失败: {e}")

@register_intent("remote_runner")
def handle_remote_runner(butler_app, entities, **kwargs):
    """处理远程运行节点指令。"""
    ...
    if not operation:
        butler_app.speak("未指定远程操作类型。")
        return

    # 获取当前连接的运行节点列表
    runners = butler_app.runner_server.list_runners()
    if not runners:
        butler_app.speak("当前没有已连接的远程运行节点。")
        return
    ...
    if not runner_id:
        if len(runners) == 1:
            ...
            # 如果有多个且未指定，尝试模糊匹配或广播
            butler_app.ui_print(f"当前可用节点: {', '.join(runners)}")
            butler_app.speak(f"检测到多个运行节点，请指定其中一个。")
            return

    # 发送指令
    success, msg = butler_app.runner_server.send_command(runner_id, operation, payload)
    if success:
        butler_app.ui_print(f"指令 '{operation}' 已发送至节点 '{runner_id}'。")
    else:
        butler_app.speak(f"发送指令失败: {msg}")

@register_intent("grep_search")
def handle_grep_search(butler_app, entities, **kwargs):
    """执行高性能全文搜索。"""
    query = entities.get("query")
    root = entities.get("root", ".")
    if not query:
        butler_app.speak("请提供搜索关键词。")
        return

    def run_grep():
        try:
            from package.core_utils.dev_tools import dev_tools
            butler_app.ui_print(f"🔍 正在全局搜索: {query} ...")
            matches = dev_tools.grep(query, root=root)
            if not matches:
                butler_app.speak(f"未在 {root} 中找到包含 '{query}' 的内容。")
            else:
                butler_app.ui_print(f"找到 {len(matches)} 处匹配:")
                for m in matches[:20]: # 仅显示前 20 条
                    butler_app.ui_print(f"  {m['file']}:{m['line']} -> {m['content'].strip()}")
                if len(matches) > 20:
                    butler_app.ui_print(f"... 以及另外 {len(matches)-20} 条记录。")
                butler_app.speak(f"搜索完成，共发现 {len(matches)} 处匹配。")
        except Exception as e:
            butler_app.speak(f"搜索失败: {e}")

    threading.Thread(target=run_grep, daemon=True).start()

@register_intent("glob_list")
def handle_glob_list(butler_app, entities, **kwargs):
    """按模式匹配列出文件。"""
    pattern = entities.get("pattern")
    if not pattern:
        butler_app.speak("请提供文件通配符模式。")
        return

    def run_glob():
        try:
            from package.core_utils.dev_tools import dev_tools
            butler_app.ui_print(f"📁 正在匹配模式: {pattern} ...")
            files = dev_tools.glob(pattern)
            if not files:
                butler_app.speak("未匹配到任何文件。")
            else:
                butler_app.ui_print(f"找到 {len(files)} 个文件:")
                for f in files[:30]:
                    butler_app.ui_print(f"  {f}")
                butler_app.speak(f"匹配完成，共找到 {len(files)} 个文件。")
        except Exception as e:
            butler_app.speak(f"匹配失败: {e}")

    threading.Thread(target=run_glob, daemon=True).start()

@register_intent("safe_edit")
def handle_safe_edit(butler_app, entities, **kwargs):
    """安全地编辑文件（搜索-替换模式）。"""
    path = entities.get("file_path")
    ...
    if not path or old_text is None or new_text is None:
        butler_app.speak("编辑操作需要文件路径、旧文本块和新文本块。")
        return

    def run_edit():
        try:
            from package.core_utils.dev_tools import dev_tools
            butler_app.ui_print(f"🛠️ 正在安全编辑文件: {path} ...")
            res = dev_tools.safe_edit(path, old_text, new_text)
            if res.get("status") == "success":
                butler_app.speak(f"文件 {path} 已成功更新。")
            else:
                butler_app.speak(f"编辑失败: {res.get('message', '未知错误')}")
        except Exception as e:
            butler_app.speak(f"编辑过程中出错: {e}")
