...
class TestButler(unittest.TestCase):
    ...
    def test_data_storage_persistence(self, mock_init_long_memory):
        ...
        def mock_save(plugin, key, val):
            storage[f"{plugin}:{key}"] = val

        def mock_load(plugin, key):
            return storage.get(f"{plugin}:{key}")

        with (
            patch.object(data_storage_manager, "save", side_effect=mock_save),
            patch.object(data_storage_manager, "load", side_effect=mock_load),
        ):
            Jarvis(self.mock_root)
            user_profile_plugin1 = extension_manager.plugin_manager.get_plugin(
                "UserProfilePlugin"
            )
            self.assertIsNotNone(
                user_profile_plugin1, "UserProfilePlugin should be loaded"
            )
            ...
            self.assertIn(
                f"Okay, I've remembered your name is {user_name}", save_result.result
            )

            # 3. Create a new Jarvis instance to simulate a restart
            Jarvis(self.mock_root)
            user_profile_plugin2 = extension_manager.plugin_manager.get_plugin(
                "UserProfilePlugin"
            )
            self.assertIsNotNone(
                user_profile_plugin2,
                "UserProfilePlugin should be loaded in the new instance",
            )
