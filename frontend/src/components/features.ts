/**
 * Butler Features Hub Component in TypeScript
 */
...
class FeaturesHub {
  public activeSessionMode: string = 'local';
  public currentProject: ProjectItem | null = null;
  ...
  // Session Mode
  public openSessionModeModal(): void {
    const modes = [
      ...
      ">
        ...
        ">
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
            ...
            <button onclick="window.FeaturesHub._closeModal()" style="background: none; border: none; color: rgba(255,255,255,0.6); cursor: pointer; font-size: 18px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 12px;">
            ${modes
              .map(
                (m) => `
              <div onclick="window.FeaturesHub._selectMode('${m.value}')" style="
  ...
  public _selectMode(mode: string): void {
    this.activeSessionMode = mode;
    this._saveToStorage();
    this._closeModal();

    const modeNames: Record<string, string> = { local: '本地', worktree: '工作树', cloud: '云端' };
    window.showToast?.('会话模式', `已切换至 ${modeNames[mode] || mode} 模式`, 'success');
  ...
  private _createWorktreeFlow(): void {
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 16px;">
              ...
              <div style="background: rgba(255,255,255,0.04); border-radius: 10px; padding: 16px;">
                ...
                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                  <button onclick="window.FeaturesHub._gitAction('branches')" style="${this._gitBtnStyle('#007AFF')}"><i class="fas fa-code-branch"></i> 查看分支</button>
                  <button onclick="window.FeaturesHub._gitAction('checkout')" style="${this._gitBtnStyle('#34C759')}"><i class="fas fa-check-circle"></i> 切换分支</button>
                </div>
              </div>
            </div>
            <div style="flex: 1; display: flex; flex-direction: column; gap: 16px;">
              <div style="background: rgba(255,255,255,0.04); border-radius: 10px; padding: 16px;">
                ...
                <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 10px;">
                  <button onclick="window.FeaturesHub._gitAction('diff')" style="${this._gitBtnStyle('#FF9500')}"><i class="fas fa-code-compare"></i> 查看变更</button>
                  <button onclick="window.FeaturesHub._gitAction('stage')" style="${this._gitBtnStyle('#34C759')}"><i class="fas fa-plus-circle"></i> 暂存全部</button>
                </div>
              ...
              <div style="background: rgba(255,255,255,0.04); border-radius: 10px; padding: 16px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px;">
                  ...
                  <div style="display: flex; gap: 8px;">
                    <button onclick="window.FeaturesHub._gitAction('commit')" style="${this._gitBtnStyle('#34C759')} flex: 1;"><i class="fas fa-check"></i> 提交</button>
                    <button onclick="window.FeaturesHub._gitAction('push')" style="${this._gitBtnStyle('#007AFF')} flex: 1;"><i class="fas fa-upload"></i> 推送</button>
                    <button onclick="window.FeaturesHub._gitAction('pr')" style="${this._gitBtnStyle('#5856D6')} flex: 1;"><i class="fas fa-code-pull-request"></i> PR</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  private _gitBtnStyle(color: string): string {
    return `
  ...
  public async _updateGitStatus(): Promise<void> {
  ...
  public async _gitAction(action: string): Promise<void> {
    const handlers: Record<string, () => Promise<any>> = {
      diff: () => this._callBackend('git', 'diff'),
      stage: () => this._callBackend('git', 'stage', { file: '.' }),
      commit: () => {
        const msgInput = document.getElementById('git-commit-msg') as HTMLInputElement;
        const msg = msgInput?.value || '更新';
  ...
  // Project Manager
  public openProjectManager(): void {
    const html = `
      <div class="features-modal-overlay" style="
          position: fixed; inset: 0; background: rgba(0,0,0,0.6);
          backdrop-filter: blur(8px); z-index: 10001;
          ...
          <div style="padding: 16px 24px;">
            <div style="display: flex; gap: 10px; margin-bottom: 16px;">
              ...
              <input type="text" id="new-project-path" placeholder="项目路径" style="
                  ...
                  border-radius: 8px; padding: 10px 14px; color: #fff;
              ">
              <button onclick="window.FeaturesHub._addProject()" style="
  ...
  public _renderProjects(): void {
    ...
    list.innerHTML = this.projects
      ...
      <div style="
          ...
          border: 1px solid ${this.currentProject?.id === p.id ? '#007AFF44' : 'rgba(255,255,255,0.06)'};
          transition: all 0.2s;
      " onclick="window.FeaturesHub._selectProject('${p.id}')">
        ...
        <button onclick="event.stopPropagation(); window.FeaturesHub._removeProject(${i})" style="
  ...
  public _addProject(): void {
    ...
    const pathInput = document.getElementById('new-project-path') as HTMLInputElement;
  ...
  public _selectProject(id: string): void {
  ...
  public _removeProject(index: number): void {
  ...
  public launchBrowser(): void {
    ...
    if (!(window as any).butlerBrowser && typeof (window as any).initBrowser === 'function') {
      (window as any).initBrowser('browser-container');
    }

    (window as any).butlerBrowser?.open();
  }

  public createPopup(): void {
    this.popupCount++;
    if ((window as any).FloatingPopupManager) {
      (window as any).FloatingPopupManager.create({
      ...
      ">
        ...
        ">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 16px;">
            <button onclick="window.FeaturesHub._computerAction('screenshot')" style="${this._actionBtnStyle('#007AFF')}">
              <i class="fas fa-camera"></i> 截屏
            </button>
            <button onclick="window.FeaturesHub._computerAction('click')" style="${this._actionBtnStyle('#34C759')}">
              <i class="fas fa-mouse-pointer"></i> 点击
            </button>
            <button onclick="window.FeaturesHub._computerAction('type')" style="${this._actionBtnStyle('#FF9500')}">
              <i class="fas fa-keyboard"></i> 输入
            </button>
            <button onclick="window.FeaturesHub._computerAction('test-gui')" style="${this._actionBtnStyle('#AF52DE')}">
  ...
  private _actionBtnStyle(color: string): string {
    return `
  ...
  public _computerAction(action: string): void {
  ...
  // Voice Dictation
  public launchVoiceDictation(): void {
    const html = `
      <div class="features-modal-overlay" style="
          position: fixed; inset: 0; background: rgba(0,0,0,0.6);
          backdrop-filter: blur(8px); z-index: 10001;
      ...
      ">
        ...
        ">
          ...
          <button onclick="window.FeaturesHub._startVoiceRecording()" style="
  ...
  public _startVoiceRecording(): void {
    this._closeModal();
      ...
      ">
        ...
        ">
          ...
          <div style="display: flex; flex-direction: column; gap: 10px;">
            <button onclick="window.FeaturesHub._worktreeAction('list')" style="${this._actionBtnStyle('#007AFF')}">
              <i class="fas fa-list"></i> 列出所有 Worktree
            </button>
            <button onclick="window.FeaturesHub._worktreeAction('create')" style="${this._actionBtnStyle('#34C759')}">
              <i class="fas fa-plus"></i> 创建新 Worktree
            </button>
            <button onclick="window.FeaturesHub._worktreeAction('prune')" style="${this._actionBtnStyle('#FF9500')}">
  ...
  public _worktreeAction(action: string): void {
    this._closeModal();
  ...
  private async _callBackend(module: string, action: string, params: Record<string, any> = {}): Promise<any> {
    try {
...
const featuresHubInstance = new FeaturesHub();

if (typeof window !== 'undefined') {
  (window as any).FeaturesHub = featuresHubInstance;
  window.featuresHub = featuresHubInstance;
}
