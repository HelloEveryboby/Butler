...
from models.models import TrafficAnalysisRecord
...
async def analyze_packets(packet_data_list: list, user_id: int, db: AsyncSession) -> dict:
    parsed_packets = []
...
def _traffic_recommendations(analysis: dict) -> list:
    recs = []
    ...
    if analysis["anomaly_detected"]:
        ...
        if any("SYN flood" in a or "flood" in a.lower() for a in details.get("anomalies_found", [])):
...
async def get_analysis_history(user_id: int, db: AsyncSession, limit: int = 50) -> list:
